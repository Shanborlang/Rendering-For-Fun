...
bool drawFrame(VulkanRenderDevice &vkDev, const std::function<void(uint32_t)> &updateBuffersFunc,
    ...
    VkResult result = vkAcquireNextImageKHR(vkDev.device, vkDev.swapchain, 0, vkDev.semaphore, VK_NULL_HANDLE,
                                            &imageIndex);
...
void VulkanRenderContext::updateBuffers(uint32_t imageIndex) {
    for (auto &r: onScreenRenderers_)
        if (r.mEnabled)
            r.mRenderer.updateBuffers(imageIndex);
}
...
void VulkanRenderContext::composeFrame(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    for (auto &r: onScreenRenderers_)
        if (r.mEnabled) {
            RenderPass rp = r.mUseDepth ? screenRenderPass : screenRenderPass_NoDepth;
            VkFramebuffer fb = (r.mUseDepth ? swapchainFramebuffers : swapchainFramebuffers_NoDepth)[imageIndex];

            if (r.mRenderer.renderPass_.handle != VK_NULL_HANDLE)
                rp = r.mRenderer.renderPass_;
            if (r.mRenderer.framebuffer_ != VK_NULL_HANDLE)
                fb = r.mRenderer.framebuffer_;

            r.mRenderer.fillCommandBuffer(commandBuffer, imageIndex, fb, rp.handle);
        }
...
void VulkanApp::assignCallbacks() {
    glfwSetCursorPosCallback(
            ...
            [](GLFWwindow *window, double x, double y) {
                ...
                const auto mx = static_cast<float>(x / width);
                const auto my = static_cast<float>(y / height);
