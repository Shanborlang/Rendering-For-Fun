...
struct Renderer {
    ...
    virtual void fillCommandBuffer(VkCommandBuffer cmdBuffer, size_t currentImage, VkFramebuffer fb = VK_NULL_HANDLE,
                                   VkRenderPass rp = VK_NULL_HANDLE) = 0;

    virtual void updateBuffers(size_t currentImage) {}

    inline void
    updateUniformBuffer(uint32_t currentImage, const uint32_t offset, const uint32_t size, const void *data) {
        UploadBufferData(ctx_.vkDev, uniforms_[currentImage].memory, offset, data, size);
    }

    void initPipeline(const std::vector<const char *> &shaders, const PipelineInfo &pInfo, uint32_t vtxConstSize = 0,
        ...
        if (!outputs.empty()) // offscreen rendering
        {
            ...
            renderPass_ = (renderPass.handle != VK_NULL_HANDLE) ? renderPass :
                          ((IsDepthFormat(outputs[0].format) && (outputs.size() == 1))
                           ? ctx_.resources.addDepthRenderPass(outputs) : ctx_.resources.addRenderPass(outputs,
    ...
    void beginRenderPass(VkRenderPass rp, VkFramebuffer fb, VkCommandBuffer commandBuffer, size_t currentImage) {
        ...
        ctx_.beginRenderPass(commandBuffer, rp, currentImage, rect,
                             ...
                             renderPass_.info.clearColor_ ? &clearValues[0] : (renderPass_.info.clearDepth_
                                                                               ? &clearValues[1] : nullptr));
