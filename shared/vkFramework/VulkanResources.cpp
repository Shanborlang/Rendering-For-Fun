...
VulkanTexture VulkanResources::loadCubeMap(const char *fileName, uint32_t mipLevels) {
    ...
    CreateImageView(vkDev.device, cubemap.image.image, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT,
                    &cubemap.image.imageView, VK_IMAGE_VIEW_TYPE_CUBE, 6, mipLevels);
...
VulkanTexture VulkanResources::loadKTX(const char *fileName) {
    ...
    CreateImageView(vkDev.device, ktx.image.image, VK_FORMAT_R16G16_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT,
                    &ktx.image.imageView);
...
VulkanTexture VulkanResources::loadTexture2D(const char *filename) {
    ...
    TransitionImageLayout(vkDev, tex.image.image, format, VK_IMAGE_LAYOUT_UNDEFINED,
...
VulkanTexture VulkanResources::addRGBATexture(int texWidth, int texHeight, void *data) {
    ...
    TransitionImageLayout(vkDev, tex.image.image, tex.format, VK_IMAGE_LAYOUT_UNDEFINED,
                          VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
VulkanTexture VulkanResources::addColorTexture(int texWidth, int texHeight, VkFormat colorFormat, VkFilter minFilter,
                                               VkFilter maxFilter, VkSamplerAddressMode addressMode) {
    ...
    TransitionImageLayout(vkDev, res.image.image, colorFormat, VK_IMAGE_LAYOUT_UNDEFINED,
                          VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
VulkanTexture VulkanResources::addDepthTexture(int texWidth, int texHeight, VkImageLayout layout) {
    ...
    if (!CreateImage(vkDev.device, vkDev.physicalDevice, w, h, depthFormat, VK_IMAGE_TILING_OPTIMAL,
    ...
    TransitionImageLayout(vkDev, depth.image.image, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED,
...
VulkanBuffer VulkanResources::addBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties,
                                        bool createMapping) {
...
VulkanBuffer
VulkanResources::addVertexBuffer(uint32_t indexBufferSize, const void *indexData, uint32_t vertexBufferSize,
                                 const void *vertexData) {
    VulkanBuffer result{};
    result.size = AllocateVertexBuffer(vkDev, &result.buffer, &result.memory, vertexBufferSize, vertexData,
...
VkDescriptorPool VulkanResources::addDescriptorPool(const DescriptorSetInfo &dsInfo, uint32_t dSetCount) {
    ...
    auto samplerCount = static_cast<uint32_t>(dsInfo.textures.size());
    ...
    if (uniformBufferCount)
        poolSizes.push_back(
                VkDescriptorPoolSize{.type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, .descriptorCount = dSetCount *
                                                                                                   uniformBufferCount});

    if (storageBufferCount)
        poolSizes.push_back(
                VkDescriptorPoolSize{.type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, .descriptorCount = dSetCount *
                                                                                                   storageBufferCount});

    if (samplerCount)
        poolSizes.push_back(
                VkDescriptorPoolSize{.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, .descriptorCount = dSetCount *
                                                                                                           samplerCount});
...
VkPipeline VulkanResources::addPipeline(VkRenderPass renderPass, VkPipelineLayout pipelineLayout,
    ...
    if (!this->createGraphicsPipeline(vkDev, renderPass, pipelineLayout, shaderFiles,
                                      &pipeline, ppInfo.topology, ppInfo.useDepth, ppInfo.useBlending,
                                      ppInfo.dynamicScissorState, ppInfo.width, ppInfo.height,
...
VkDescriptorSetLayout VulkanResources::addDescriptorSetLayout(const DescriptorSetInfo &dsInfo) {
    ...
    for (const auto &i: dsInfo.textures) {
        bindings.push_back(
                DescriptorSetLayoutBinding(bindingIdx++, i.dInfo.type /*VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER*/,
...
VkDescriptorSet
VulkanResources::addDescriptorSet(VkDescriptorPool descriptorPool, VkDescriptorSetLayout dsLayout) const {
...
*/
void VulkanResources::updateDescriptorSet(VkDescriptorSet ds, const DescriptorSetInfo &dsInfo) const {
    uint32_t bindingIdx = 0;
    ...
    for (size_t ta = 0; ta < dsInfo.textureArrays.size(); ta++) {
        ...
        for (auto t: dsInfo.textureArrays[ta].textures) {
...
RenderPass VulkanResources::addRenderPass(const std::vector<VulkanTexture> &outputs, const RenderPassCreateInfo &ci,
                                          bool useDepth) {
    ...
    if (outputs.size() == 1) {
        ...
        if (!CreateColorAndDepthRenderPass(vkDev, useDepth && (outputs.size() > 1), &renderPass, ci,
                                           outputs[0].format)) {
...
RenderPass
VulkanResources::addDepthRenderPass(const std::vector<VulkanTexture> &outputs, const RenderPassCreateInfo &ci) {
    VkRenderPass renderPass;
...
VkPipelineLayout
VulkanResources::addPipelineLayout(VkDescriptorSetLayout dsLayout, uint32_t vtxConstSize, uint32_t fragConstSize) {
...
VulkanTexture VulkanResources::createFontTexture(const char *fontFile) {
    ...
    if (!pixels ||
        !CreateTextureImageFromData(vkDev, res.image.image, res.image.imageMemory, pixels, texWidth, texHeight,
                                    VK_FORMAT_R8G8B8A8_UNORM)) {
        printf("Failed to load texture\n");
        return res;
    }

    CreateImageView(vkDev.device, res.image.image, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT,
                    &res.image.imageView);
...
VulkanResources::makeMeshBuffers(const std::vector<float> &vertices, const std::vector<unsigned int> &indices) {
    const auto indexBufferSize = uint32_t(indices.size() * sizeof(int));
    const auto vertexBufferSize = uint32_t(vertices.size() * sizeof(float));
...
std::pair<BufferAttachment, BufferAttachment>
VulkanResources::loadMeshToBuffer(const char *filename, bool useTextureCoordinates, bool useNormals,
    ...
    const auto vertexBufferSize = static_cast<uint32_t>(sizeof(float) * vertices.size());
    const auto indexBufferSize = static_cast<uint32_t>(sizeof(unsigned int) * indices.size());
