...
struct RenderItem {
    ...
    explicit RenderItem(Renderer &r, bool useDepth = true)
            : mRenderer(r), mUseDepth(useDepth) {}
};
...
struct VulkanRenderContext {
    ...
    VulkanRenderContext(void *window, uint32_t screenWidth, uint32_t screenHeight,
            ...
            finalRenderPass(resources.addFullScreenPass(true,
                                                        RenderPassCreateInfo{.clearColor_ = false, .clearDepth_ = false, .flags_ = eRenderPassBit_Last})),
            clearRenderPass(resources.addFullScreenPass(true,
...
struct VulkanApp {
    ...
    [[nodiscard]] static inline bool shouldHandleMouse() { return !ImGui::GetIO().WantCaptureMouse; }
    ...
    [[nodiscard]] inline float getFPS() const { return fpsCounter_.GetFPS(); }
...
protected:
    ...
    std::vector<RenderItem> &onScreenRenderers_;
    FramePerSecondCounter fpsCounter_;
