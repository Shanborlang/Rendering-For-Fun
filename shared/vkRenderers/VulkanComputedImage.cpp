#include "VulkanComputedImage.h"

ComputedImage::ComputedImage(VulkanRenderDevice &vkDev, const char *shaderName, uint32_t textureWidth,
                             uint32_t textureHeight, bool supportDownload)
    ...
    VK_CHECK(vkCreatePipelineLayout(vkDev.device, &pipelineLayoutCreateInfo, nullptr, &pipelineLayout));
...
bool ComputedImage::CreateComputedTexture(uint32_t computedWidth, uint32_t computedHeight, VkFormat format) {
    ...
    if(!CreateImage(vkDev.device, vkDev.physicalDevice, computedWidth, computedHeight, format,
                    VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT |
                            (canDownloadImage ? VK_IMAGE_USAGE_TRANSFER_SRC_BIT : 0), !canDownloadImage ? VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT
                            : (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT), computed.image, computed.imageMemory))
        return false;
...
bool ComputedImage::CreateDescriptorSet() {
    ...
    vkUpdateDescriptorSets(vkDev.device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0,
                           nullptr);
