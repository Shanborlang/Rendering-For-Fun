...
void initMesh() {
  ...
  imgGen =
	  std::make_unique<ComputedImage>(vkDev, "../../../data/shaders/VK04_compute_texture.comp", 1024, 1024, false);
  meshGen = std::make_unique<ComputedVertexBuffer>(vkDev,
												   "../../../data/shaders/VK04_compute_mesh.comp",
												   indexBufferSize,
  ...
  std::vector<const char *>
	  shaders = {"../../../data/shaders/VK04_render.vert", "../../../data/shaders/VK04_render.frag"};
  std::vector<const char *>
	  shadersColor = {"../../../data/shaders/VK04_render.vert", "../../../data/shaders/VK04_render_color.frag"};
...
void renderGUI(uint32_t imageIndex) {
  ...
  ImGui::Begin("Torus Knot params", nullptr);
  {
	...
	for (const auto &i: PQ) {
	  std::string title = std::to_string(i.first) + ", " + std::to_string(i.second);
	  if (ImGui::Button(title.c_str())) {
		if (i != morphQueue.back())
		  morphQueue.push_back(i);
	  }
	}
  }
  ImGui::End();
